...
function App() {
  ...
  return <div className="p-2 mb-2 max-md:m-0 max-md:mb-20 max-md:p-2 w-[100%] ">
     <div className={`mb-1  p-2 gap-1 rounded flex justify-around md:hidden`}>
      ...
      }} className={`${path.includes('upcomming')?'p-2 font-medium text-md  bg-blue-50 rounded-lg w-[100%]':'p-2 text-md font-normal bg-white rounded-lg w-[100%]'}`}>Upcomming</button>
      <button onClick={()=>{
        router.push('/medvisit/completed')
      }} className={`${path.includes('completed')?'p-2 text-md font-medium bg-blue-50 rounded-lg w-[100%]':'p-2 text-md font-normal bg-white rounded-lg w-[100%]'}`}>Completed</button>
